...
void assert_point_equal(const Point<3> &a, const Point<3> &b, double eps = EQ_APPROX) {
    for (int i = 0; i < 3; i++) {
...
void assert_vector_equal(const Vector<3> &a, const Vector<3> &b, double eps = EQ_APPROX) {
    for (int i = 0; i < 3; i++) {
        cr_assert_float_eq(a.m_components[i], b.m_components[i], eps,
                          "Component %d mismatch: expected %f, got %f", i,
                          b.m_components[i], a.m_components[i]);
...
Test(RotationSuite, RotateArbitraryPoint) {
    ...
    Point<3> p1 = tx.transformPoint(p);
    Point<3> p2 = ty.transformPoint(p1);
    Point<3> p3 = tz.transformPoint(p2);
