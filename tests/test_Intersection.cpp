...
public:
    ...
    Raytracer::Core::Color computeColor(
        const Intersection&,
        const Raytracer::Core::Ray&,
        const std::vector<std::shared_ptr<Raytracer::Core::ILight>>&) const override {
...
void assert_point3d_eq(const Point<3>& p1, const Point<3>& p2, double eps = EQ_APPROX) {
    for (std::size_t i = 0; i < 3; ++i) {
        cr_assert_float_eq(p1.m_components[i], p2.m_components[i], eps,
                        "component[%zu]: expected %g but got %g", i, p2.m_components[i],
                        p1.m_components[i]);
    }
}

void assert_vector3d_eq(const Vector<3>& v1, const Vector<3>& v2, double eps = EQ_APPROX) {
    for (std::size_t i = 0; i < 3; ++i) {
        cr_assert_float_eq(v1.m_components[i], v2.m_components[i], eps,
                        "component[%zu]: expected %g but got %g", i, v2.m_components[i],
                        v1.m_components[i]);
    }
}

void assert_point2d_eq(const Point<2>& p1, const Point<2>& p2, double eps = EQ_APPROX) {
