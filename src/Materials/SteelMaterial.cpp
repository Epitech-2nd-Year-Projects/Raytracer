 ...
 * @brief Implementation of the SteelMaterial class, providing brushed aluminum effect.
 */
...
Math::Vector<3> SteelMaterial::randomInUnitSphere() const {
    ...
    while (randomNoise.squaredNorm() >= 1.0) {
        randomNoise = Math::Vector<3>(
            m_distribution(m_generator) * 0.2,
...
Core::Color SteelMaterial::computeColor(const Core::Intersection &intersection,
    const Core::Ray &ray,
  ...
  double specularIntensity = std::max(0.0, std::pow(-ray.getDirection().dot(reflectDir), 10.0)) * 0.4;
  Core::Color specularColor = Core::Color(255, 255, 255) * specularIntensity;

  if (!newIntersection)
    return aluminumBaseColor.add(specularColor).add(getAmbientColor() * getAmbientCoefficient());
