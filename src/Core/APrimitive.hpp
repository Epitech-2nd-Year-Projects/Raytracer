...
public:
  ...
  void setPosition(const Math::Point<3> &p) noexcept { m_position = p; }
  void setRotation(const Math::Vector<3> &r) noexcept { m_rotation = r; }
  void setScale(const Math::Vector<3> &s) noexcept { m_scale = s; }
  ...
  [[nodiscard]] const std::unique_ptr<IMaterial> &getMaterial() const noexcept {
    return m_material;
  }

  void transform(const Math::Point<3> &p, const Math::Vector<3> &r,
                 const Math::Vector<3> &s) noexcept {
    setPosition(p);
    setRotation(r);
    setScale(s);
  }
