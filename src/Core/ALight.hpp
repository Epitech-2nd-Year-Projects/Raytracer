...
public:
  ...
  [[nodiscard]] constexpr double getIntensity() const noexcept override { return m_intensity.get(); }
  [[nodiscard]] constexpr const Color &getColor() const noexcept override { return m_color; }
...
public:
  ...
  void setDirection(const Math::Vector<3> &direction) noexcept override {
    m_direction = direction.length() > 0 ? direction / direction.length() : Math::Vector<3>{0, -1, 0};
  }
  ...
  [[nodiscard]] Math::Vector<3> getDirectionFrom([[maybe_unused]] const Math::Point<3> &point) const noexcept override {
    return m_direction;
  }

  [[nodiscard]] double computeIllumination(const Math::Point<3> &intersectionPoint, const Math::Vector<3> &normal) const noexcept override {
...
public:
  ...
  void setPosition(const Math::Point<3> &p) noexcept override { m_position = p; }
  [[nodiscard]] constexpr const Math::Point<3> &getPosition() const noexcept override { return m_position; }

  [[nodiscard]] double getDistance(const Math::Point<3> &point) const noexcept override {
    const Math::Vector<3> diff = point - m_position;
    return diff.length();
  }

  [[nodiscard]] Math::Vector<3> getDirectionFrom(
    const Math::Point<3> &point) const noexcept override {
  ...
  [[nodiscard]] double computeIllumination(
    const Math::Point<3> &intersectionPoint,
