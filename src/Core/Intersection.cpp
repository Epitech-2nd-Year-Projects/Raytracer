...
Intersection::Intersection(const Math::Point<3> &point, const Math::Vector<3> &normal,
...
double Intersection::getDistance() const noexcept {
  return m_distance;
}

bool Intersection::getIsInside() const noexcept {
  return m_isInside;
}
...
const Math::Point<2>& Intersection::getUv() const noexcept {
  return m_uv;
}
}